...
// Physical left->right sensor order is A4, A3, A2, A1.
constexpr uint8_t SENSOR_PINS[4] = {A4, A3, A2, A1};
constexpr float SENSOR_WEIGHTS[4] = {-3.0f, -1.0f, 1.0f, 3.0f};

// Measured calibration points (left->right: A4, A3, A2, A1).
constexpr int SENSOR_WHITE[4] = {713, 434, 473, 519};
constexpr int SENSOR_BLACK[4] = {998, 979, 987, 990};
...
void readSensors(float lineStrength[4], int rawOut[4]) {
  for (int i = 0; i < 4; i++) {
...
void setup() {
  ...
  Serial.println(F("line_follow ready"));
  Serial.println(F("sensor order left->right: A4 A3 A2 A1"));
}
